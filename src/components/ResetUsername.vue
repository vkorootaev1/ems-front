    ...
    <div class="containter custom-modal-wrap" @mousedown="$emit('close')">
        <div class="row align-items-center justify-content-center mt-5 mx-2">
            <div class="col-lg-6 col-md-8 col-12 custom-modal-body" @mousedown.stop="">
                <h4>Восстановление имени пользователя</h4>
                <input type=" text" v-model="email" @keyup.enter="resetUsername" placeholder="Email" class="form__input">
                <div class="row justify-content-center">
                    <div class="col">
                        <input type="submit" @click="resetUsername" value="Восстановить" class="form__btn w-100">
                    </div>
                    <div class="col">
...
<script setup>
import { ref } from 'vue';
import { resetUsernameAPI } from '@/api/auth'
...
const error_message_reset_username = 'Не удалось восстановить имя пользователя'
...
const resetUsername = async () => {
    if (validateEmail()) {
        try {
            await resetUsernameAPI(email.value)
            ...
            try {
                ...
                errors.value.push(error_message_reset_username)
            }
        }
    }
}
